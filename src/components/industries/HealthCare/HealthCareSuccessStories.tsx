...
import { HealthSuccessStoriesData } from "./HealthCareSuccessStoriesData";

export default function HealthCareSuccessStories() {
  const { id } = useParams();
  const [data, setData] = useState<(typeof HealthSuccessStoriesData)[0] | null>(
    null
  );

  useEffect(() => {
    if (id) {
      setData(HealthSuccessStoriesData[parseInt(id)]);
    }
  }, [id]);
