...
import { GovernmentSuccessStoriesData } from "./GovernmentSuccessStoriesData";

export default function GovernmentSuccessStories() {
  const { id } = useParams();
  const [data, setData] = useState<
    (typeof GovernmentSuccessStoriesData)[0] | null
  >(null);

  useEffect(() => {
    if (id) {
      setData(GovernmentSuccessStoriesData[parseInt(id)]);
    }
  }, [id]);
