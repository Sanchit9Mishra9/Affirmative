...
import { OilAndGasSuccessStoriesData } from "./OilAndGasSuccessStoriesData";

export default function OilAndGasSuccessStories() {
  const { id } = useParams();
  const [data, setData] = useState<
    (typeof OilAndGasSuccessStoriesData)[0] | null
  >(null);

  useEffect(() => {
    if (id) {
      setData(OilAndGasSuccessStoriesData[parseInt(id)]);
    }
  }, [id]);
