...
import { ADSuccessStoriesData } from "./ADSuccessStoriesData";
import { useEffect, useState } from "react";

export default function AerospaceSuccessStories() {
  const { id } = useParams();
  const [data, setData] = useState<(typeof ADSuccessStoriesData)[0] | null>(
    null
  );

  useEffect(() => {
    if (id) {
      setData(ADSuccessStoriesData[parseInt(id)]);
    }
  }, [id]);
